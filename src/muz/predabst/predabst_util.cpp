...
namespace predabst {
    ...
    expr_ref model_eval(expr_ref const& e, model_evaluator& ev, bool model_completion) {
        ...
        try {
            ...
            (void)ex;
            STRACE("predabst", tout << "Evaluation of " << e << " failed: " << ex.msg() << "\n";);
            throw default_exception("model evaluation failed");
        }
    }
}
