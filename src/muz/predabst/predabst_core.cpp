...
namespace predabst {
    ...
    public:
        imp(vector<symbol_info*> const& symbols, vector<rule_info*> const& rules, expr_ref_vector const& template_param_values, fixedpoint_params const& fp_params, cancellation_manager& cm, ast_manager& m, core_stats& stats) :
            ...
            if (m_fp_params.predabst_no_simplify()) {
    ...
    private:
        ...
        void cart_pred_abst_rule(rule_info const* ri, unsigned fixed_pos = 0, node_info const* fixed_node = NULL) {
            ...
            if (m_fp_params.predabst_bail_if_no_combinations() && (found_combs == all_combs)) {
            ...
            if (m_fp_params.predabst_summarize_cubes()) {
                for (unsigned i = 0; i < all_cubes.size(); ++i) {
                    ...
                    for (unsigned j = 0; j < pos_cubes.size(); ++j) {
                        ...
                        if (m_fp_params.predabst_summarize_cubes_using_iff()) {
            ...
            if (m_fp_params.predabst_use_allsat()) {
                for (unsigned i = 0; i < all_cubes.size(); ++i) {
                ...
                while (check(solver_for(ri)) == l_true) {
                    ...
                    if (m_fp_params.predabst_bail_if_all_combinations_sat() && (found_combs == all_combs)) {
        ...
        vector<unsigned> get_rule_position_ordering(vector<vector<T>> const& sizes) const {
            ...
            if (m_fp_params.predabst_order_cartprod_choices()) {
            ...
            if (chosen_nodes.size() < positions.size()) {
                ...
                for (node_vector::const_iterator it = pos_nodes.begin(); it != pos_nodes.end(); ++it, ++j) {
                    ...
                    if ((assume_sat && (all_cubes[i].size() == 1)) || (pos_cube.size() == 0)) {
                        ...
                        if (m_fp_params.predabst_use_body_assumptions()) {
        ...
        cube_t cart_pred_abst_cube(rule_info const* ri, expr_ref_vector const& es, expr_ref_vector& assumptions) {
            ...
            else if (m_fp_params.predabst_skip_known_head_values() && (info->m_num_head_unknown_args == 0)) {
