...
namespace predabst {
    ...
    expr_ref normalize_pred(expr_ref const& e, var_ref_vector const& vars) {
        ...
        th_rewriter tw(m);
        expr_ref e2 = e;
        tw(e2); // This will convert 0 = 0 to true, for example.
        e2 = to_nnf(e2);
        ...
        // implementation of to_nnf happens to do as well.  But it will also convert
        // (distinct x y) into (or (< x y) (< y x)), which is probably undesirable.
        for (unsigned i = 0; i < vars.size(); ++i) {
            if (!sort_is_int(vars.get(i), m)) {
